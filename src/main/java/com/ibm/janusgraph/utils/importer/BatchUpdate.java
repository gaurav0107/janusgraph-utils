...
public class BatchUpdate {
    ...
    public static void main(String args[]) throws Exception {
        if (null == args || args.length < 3) {
            System.err.println(
                    "Usage: BatchUpdate <janusgraph-config-file> <data-files-directory> <data-mapping.json>");
            System.exit(1);
        }
        ...
        new DataLoader(graph).updateVertex(args[1], args[2]);
        new DataLoader(graph).updateEdges(args[1], args[2]);
        graph.close();
    }
}
